						...
						<div class="col-sm-12 profile-field-div editable-div">
							<c:choose>
								<c:when test='${empty professionalProfile.currentEmployer}'>
									<span class="inline-output" style="color: orange">Who is
										your current employer?</span>
								</c:when>
								<c:otherwise>
									<span class="inline-output"><c:out
								...
								<h5 style="font-family: calibri">
									From <span class="start-month"> <c:out
										...
										<c:out value='${item.startYear}' />
											...
											value='${item.endMonth}' /></span> <span class="end-year"> <c:out
											value='${item.endYear}' /></span>
								</h5>
					...
					<div class="col-sm-12 editable-div">
						...
						<div class="inline-input">
							<form class="education-form">
								...
								<div>
									<label>Certificate Earned:</label><br /> <input placeholder=""
										name="certificate" id="certificate" style="width: 70%;">
								</div>
								<div>
		...
		<div class="col-sm-4">
				...
				<div class="col-sm-12">
					...
					<c:choose>
						<c:when test="${empty uppb.cv}">
							...
							<p>
								<a class="upload-link upload-link-1" href="#">Upload CV</a>
								<a class="upload-link upload-link-2" href="#" style="display: none;">Change</a>
								<input type="hidden" name="old-key" id="old-key">
							</p>
						</c:when>
						<c:otherwise>
							<p class="file-name">
								<a target="_blank" href="<c:url value='/bq/close/get-cv?safe-key=${uppb.cvSafeString}' />"><c:out value='${uppb.cv}' /></a>
							</p>
							<p>
			...
			<div class="col-sm-12  no-padding-div">
				<iframe class="dashboard-section" style="margin: 0"
					src="/images/ad2/bootcamp_ext.html"
					width="336" height="280" scrolling="no" frameBorder='0'></iframe>
			</div>
			<div class="col-sm-12  no-padding-div">
									...
									function(e) {
										...
										$.ajax({
											...
											success : function(data) {
												...
												$(".file-name").html("<a target='_blank' href='<c:url value='/bq/close/get-cv?safe-key="+data.safeKey+"' />'>"+data.fileName+"</a>");
												$(".upload-link-1").hide();
												$(".upload-link-2").show();
												$("#old-key").val(data.safeKey);
